...
uc(:,1)= -(-muf*(norm(v(:,1))^(2*(ef-1)))*v(:,1)...
         -(-1+kappaf*dist(x(:,1),riff)^2)*logh(x(:,1),riff));
...
for k=1:n
    ...
    uc(:,k+1)=1/h*(trasp(z(:,k+1),x(:,k+1),w(:,k+1))-trasp(x(:,k),x(:,k+1),trasp(z(:,k),x(:,k),w(:,k))))...
                   -(-muf*(norm(v(:,k))^(2*(ef-1)))*v(:,k)...
                   -(-1+kappaf*dist(x(:,k),riff)^2)*logh(x(:,k),riff));
...
print Sincr_HardSoft_random_uc -dpdf -r500;
...
%grafico energia alta qualità
print Sincr_HardSoft_random_uc_energy -dpdf -r500;